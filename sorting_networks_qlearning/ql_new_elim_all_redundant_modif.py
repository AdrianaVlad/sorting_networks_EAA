...
class LayeredSortingNetworkEnv:
    ...
    def _get_state_representation(self):
        compare_matrix = [[0] * self.n_wires for _ in range(self.n_wires)]

        for layer in self.current_layers:
            for ((i, j), _) in layer:
                compare_matrix[i][j] = 1
                compare_matrix[j][i] = 1  # comparare simetrica

        flat = tuple([bit for row in compare_matrix for bit in row])
        return flat
...
class QLearningAgent:
    def __init__(self, n_wires, max_comparators, learning_rate=0.01, discount_factor=0.99, exploration_rate=1.0, buffer_size=10000):
    ...
    def choose_action(self, state):
        ...
        valid_actions = [(i, j) for (i, j) in valid_actions if self.env.comparator_has_effect(i, j)] # optimizare
    ...
    def update_q_value(self, state, action, reward, next_state):
        ...
        self._update_single_transition(state, action, reward, next_state)

        batch_size = 32
        if len(self.replay_buffer) >= batch_size:
            batch = random.sample(self.replay_buffer, batch_size)
            for s, a, r, next_s in batch:
                self._update_single_transition(s, a, r, next_s)

    def _update_single_transition(self, state, action, reward, next_state):
        next_valid_actions = self.env.get_valid_actions()
        max_next_q = (
            max(self.get_q_value(next_state, a) for a in next_valid_actions)
            if next_valid_actions else 0.0
...
if __name__ == "__main__":
    ...
    best_network, best_size = find_layered_sorting_network(n_wires, max_comparators, episodes=episodes)
