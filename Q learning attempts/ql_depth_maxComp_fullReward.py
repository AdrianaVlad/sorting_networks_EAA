...
class LayeredSortingNetworkEnv:
    ...
    def step(self, action, best_network_size):
        ...
        elif sum(len(layer) for layer in self.current_layers) >= self.max_comparators:
            return self._get_state_representation(), -10, True
        else:
            return self._get_state_representation(), -1, False
...
class QLearningAgent:
    def __init__(self, n_wires, max_comparators, learning_rate=0.1, discount_factor=0.99, exploration_rate=0.95):
