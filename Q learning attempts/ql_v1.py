...
class SortingNetworkEnv:
    ...
    def step(self, action, best_network_size):
        ...
        is_valid_sorting_network = self._check_sorting_network()

        if is_valid_sorting_network:
            self.simplify_network()
            if len(self.current_comparators) < best_network_size: 
                reward = 1000
                done = True
            else:
                reward = 100
                done = True
        elif len(self.current_comparators) >= self.max_comparators:
            reward = -100
            done = False
        else:
            reward = -1
