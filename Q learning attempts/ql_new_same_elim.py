    ...
    def step(self, action, best_network_size):
        if action == "NEXT_LAYER":
            if sum(len(layer) for layer in self.current_layers) < self.max_comparators:
                ...
                self.best_layers.append([])
                self.current_layers[self.depth] = sorted(self.current_layers[self.depth])
                self.best_layers[self.depth] = sorted(self.best_layers[self.best_depth])
                self.depth += 1
                self.best_depth += 1
                return self._get_state_representation(), -5, False, self._get_best_state_representation()
            else:
                return self._get_state_representation(), -10, True, self._get_best_state_representation()
        ...
        if not self.comparator_has_effect(i, j):
            return self._get_state_representation(), -5, False, self._get_best_state_representation()
        else:
            self.best_layers[self.best_depth].append((i, j))
        ...
        if is_valid:
            ...
            reward = 100.0 / total_comparators
            done = True
        else:
            reward = 10.0 * partial_score / total_comparators

        return self._get_state_representation(), reward, done, self._get_best_state_representation()

    def _get_state_representation(self):
        return tuple(tuple(layer) for layer in self.current_layers), self.depth
    def _get_best_state_representation(self):
        return tuple(tuple(layer) for layer in self.best_layers), self.best_depth
...
class QLearningAgent:
    ...
    def choose_action(self, state, best_state):
    ...
    def train(self, n_episodes=1000):
        ...
        for episode in range(n_episodes):
            ...
            while not done:
                ...
                next_state, reward, done, best_next_state = self.env.step(action, best_size)
                self.update_q_value(state, action, reward, next_state)
                self.update_q_value(best_state, action, reward, best_state)
                state = next_state
                best_state = best_next_state

            total_comparators = sum(len(layer) for layer in self.env.current_layers)
            total_best_comparators = sum(len(layer) for layer in self.env.best_layers)
            if reward > 0 and total_comparators < best_size:
                best_size = total_comparators
                best_best_size = total_best_comparators
            ...
            if episode % 100 == 0:
                print(f"episode {episode}, best: {best_size}/{best_best_size}, exploration rate: {self.exploration_rate:.4f}")
